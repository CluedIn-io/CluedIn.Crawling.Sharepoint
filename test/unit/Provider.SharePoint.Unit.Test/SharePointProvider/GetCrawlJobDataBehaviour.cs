...
{
    public class GetCrawlJobDataBehaviour : SharePointProviderTest
    {
        ...
        [Theory]
        [InlineAutoData(SharePointConstants.KeyName.Url, nameof(SharePointCrawlJobData.Url))]
        ...
        [Theory]
        [InlineAutoData]
        public async Task SharePointCrawlJobDataReturned(Dictionary<string, object> dictionary, Guid organizationId, Guid userId, Guid providerDefinitionId)
        {
            Assert.IsType<CluedIn.Crawling.SharePoint.Core.SharePointCrawlJobData>(
                await Sut.GetCrawlJobData(_context, dictionary, organizationId, userId, providerDefinitionId));
        }
    }
}
